...
export default function LeaderboardPage() {
  ...
  const userBestWpm = matchHistory.length > 0 ? Math.max(...matchHistory.map(r => r.wpm)) : 0;
  ...
  // Build ranked list: ghost players + user (if they've played)
  const entries = [...GHOST_PLAYERS.map(g => ({ ...g, isUser: false }))];
  if (userBestWpm > 0) {
    entries.push({
      id: 'user',
