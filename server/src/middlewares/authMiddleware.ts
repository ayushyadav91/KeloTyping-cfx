...
export const socketAuthMiddleware = async (
  ...
  try {
    ...
    if (token === env.MOCK_AUTH_TOKEN || (env.NODE_ENV !== 'production' && !token)) {
      socket.data.user = { id: 'mock_user_101', username: 'User' };
      logger.debug('Socket authenticated via mock token / dev fallback', { socketId: socket.id });
      return next();
    }
